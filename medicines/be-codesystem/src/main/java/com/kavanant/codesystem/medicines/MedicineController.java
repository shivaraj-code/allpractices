...
public class MedicineController {
	...
	public ResponseEntity<Page<Medicine>> getMedicineByPagination(@RequestParam(defaultValue = "10") Integer pageSize,
	...
	@GetMapping(path = "/medicine/search")
	public ResponseEntity<Page<Medicine>> getNDCorName(@RequestParam String NDCorName,
			@RequestParam(defaultValue = "10") Integer pageSize, @RequestParam(defaultValue = "0") Integer pageNumber) {
		Pageable pageable = PageRequest.of(pageNumber, pageSize);
		HttpHeaders headers = new HttpHeaders();
		Page<Medicine> medResponse = medicineservice.getNDCorName(NDCorName, pageable);
	...
	public ResponseEntity<Medicine> addMedicine(@RequestBody Medicine medrequest) {
		...
		HttpHeaders headers = new HttpHeaders();
		return new ResponseEntity<Medicine>(medresponse, headers, HttpStatus.OK);
	}
